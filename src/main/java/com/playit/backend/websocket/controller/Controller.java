...
public abstract class Controller {
	...
	public abstract void handleRequest(WebSocketSession session, JsonObject data, PlayITService playITService)
	    throws Exception;

	protected void userHasRoleOrThrow(WebSocketSession session, SessionRole role) throws RoleUtilisateurException {
		SessionRole userRole = (SessionRole) session.getAttributes()
		                                            .get("role");
		if (userRole != role) {
